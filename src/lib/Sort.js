...
export const Sort = (props) => {
  ...
  const setAsc = () =>
    setSort({ options: props.options.sort(ascCompare), direction: 'asc' });
  const setDesc = () =>
    setSort({ options: props.options.sort(descCompare), direction: 'desc' });
  const color = (direction) =>
    sort.direction === direction ? 'brand' : undefined;

  return (
    <Box direction="row" justify="start" align="center" responsive={false}>
      <Select {...props} />
      <Box direction="row" flex={false} responsive={false} align="center">
        <Button icon={<Ascend color={color('asc')} />} onClick={setAsc} />
        <Button icon={<Descend color={color('desc')} />} onClick={setDesc} />
      </Box>
    </Box>
  );
};
